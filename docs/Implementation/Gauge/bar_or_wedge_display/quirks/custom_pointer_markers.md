...
For the current GoGauges `bar` gauge, the behaviour applies to rendered marker state associated with the displayed displayed fill or reveal extent. It must not change source sensor values, persisted log output, exported values, or configured range semantics.
