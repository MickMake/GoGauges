...
For the current GoGauges `bar` gauge, the behaviour applies to displayed state only. It must not alter the input sensor value, configured range, exported values, or logs.
